...
def argmin_dych_function_cost(tuple_points_arg, tuple_gradient_arg, alpha):
    new_arguments = []
    ...
    return function_cost(new_arguments)
...
def argmin_dych(tuple_points_arg, tuple_gradient_arg, number_of_iterations):
    a = DYCH_LEFT
    b = DYCH_RIGHT
    for k in range(number_of_iterations):
        ...
        x1 = m - 0.00000001
        x2 = m + 0.00000001
        fx1 = argmin_dych_function_cost(tuple_points_arg, tuple_gradient_arg, x1)
        fx2 = argmin_dych_function_cost(tuple_points_arg, tuple_gradient_arg, x2)
        if fx1 <= fx2:
            b = x2
        else:
            a = x1
